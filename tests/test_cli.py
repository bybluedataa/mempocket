...
from pcms.cli import cli
from pcms.store import init_storage, save_entry
from pcms.models import Entry
from pcms.config import Entity, Context
...
class TestNewCommand:
    def test_new_project(self, runner, temp_pcms_home):
    ...
    def test_new_without_entity(self, runner, temp_pcms_home):
        result = runner.invoke(cli, [
            "new", "Test", "--work"
        ])
        assert result.exit_code == 0
    ...
    def test_new_without_context(self, runner, temp_pcms_home):
        result = runner.invoke(cli, [
            "new", "Test", "--project"
        ])
        assert result.exit_code == 0
...
class TestListCommand:
    def test_list_empty(self, runner, temp_pcms_home):
        result = runner.invoke(cli, ["list"])
        assert result.exit_code == 0
        assert "No entries found" in result.output

    def test_list_with_entries(self, runner, temp_pcms_home):
        entry = Entry(
            title="Test Entry",
    ...
    def test_list_filter_entity(self, runner, temp_pcms_home):
...
class TestSearchCommand:
    def test_search(self, runner, temp_pcms_home):
        entry = Entry(
    ...
    def test_search_no_results(self, runner, temp_pcms_home):
...
class TestGetCommand:
    def test_get_entry(self, runner, temp_pcms_home):
        entry = Entry(
            title="Test Entry",
            entity=Entity.PROJECT,
            context=Context.WORK,
    ...
    def test_get_not_found(self, runner, temp_pcms_home):
...
class TestStatusCommand:
    def test_status(self, runner, temp_pcms_home):
        result = runner.invoke(cli, ["status"])
        assert result.exit_code == 0
        assert "PCMS Status" in result.output
        assert "Total entries" in result.output


class TestPendingCommand:
    def test_pending_empty(self, runner, temp_pcms_home):
